...
- [ ] AC1: Given an authenticated caller, when they create a project with valid input, then it is persisted and returned with its identifier.
- [ ] AC2: Given an unauthenticated or invalid-token caller, when they attempt either operation, then the API returns 401 and nothing is persisted.
...
- Create a project with a valid name, then list: it appears.
...
- **Who may create a project** depends on the global role set, which is unanswered (`PROJECT.md` Q7). The ticket can be implemented with "any authenticated caller" and tightened later, but that is a deliberate choice refinement should make explicitly rather than by default.
