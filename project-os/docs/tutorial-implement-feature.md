...
git commit -am "os: T-0005 claimed by claude-eng-4f2a"
...
```bash
git worktree add ../golinks--t-0005-create-short-link t-0005-create-short-link
```

Source work happens in that worktree on the ticket branch (`T-0005: add redirect handler` commits); the primary checkout stays on `main` for anything process-shaped ([GIT.md](../standards/GIT.md)). During the work:
...
4. Merge (squash, `T-0005: employee-created go-links with redirect`), delete branch + worktree.
5. On the trunk: `status: in-acceptance`, `implemented_by: claude-eng-4f2a`, owner cleared — `os: T-0005 in-acceptance`.
...
`/complete-ticket` (same QA session): walks the [DoD](../governance/DEFINITION_OF_DONE.md) literally — universal items, then applicable conditionals (ADR-0004 Accepted? docs updated? migration tested?). Then one finalize commit: `status: done`, `accepted_by: claude-qa-b81d`, DoD checkbox ticked, sprint row done, backlog row moved to Completed, dependents unblocked. Checkpoint:

```bash
python3 tools/validate-project-os/validate.py   # OK — every gate left a trace
git log --oneline | head                        # readable delivery journal:
# os: T-0005 done - accepted by claude-qa-b81d
# T-0005: employee-created go-links with redirect
# os: ADR-0004 accepted
# os: T-0005 claimed by claude-eng-4f2a
# os: plan SPRINT-001
```
