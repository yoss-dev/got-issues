...
- **This project's defects have been in persistence, which is what a repository interface substitutes away.** *(Corrected 2026-08-31, before acceptance: the first draft of this bullet said "the test strategy does not want mocks" and that this project "has decided it does not rely on" them. That is false, and the maintainer caught it. [TESTING.md](../../standards/TESTING.md) never mentions mocks; it requires that "new logic gets unit tests", it explicitly sanctions a test authentication handler — a double this suite has used since [T-0009](../../product/tickets/T-0009-role-authorisation-and-user-projection.md) — and the one thing it bans is narrower and better argued: the EF in-memory **provider**, because it "does not enforce constraints or translate real SQL". An ADR arguing against unrecorded assumptions is a poor place to assert one.)*

  The argument that survives is about evidence rather than prohibition. A repository interface's main payoff in tests is substituting persistence — and every defect the last two tickets produced was **in** persistence: a migration backfilling `0` into an existing column, a control character PostgreSQL cannot store, an `UPDATE … RETURNING` whose atomicity is the entire correctness argument, a unique index that turns silent duplication into a loud failure. A double that stood in for the database would have reported success for all four. That is a reason to be careful about *what* is substituted here; it is not a reason to avoid test doubles, and it says nothing against unit-testing logic that does not touch the database.
